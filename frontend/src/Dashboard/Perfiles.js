    ...
    return (
        <Box sx={{ backgroundColor: 'white', padding: 4, width: 900, display: 'block', justifyContent: 'center', margin: 0 }}>
            ...
            <Dialog open={open} onClose={handleCloseDeleteModal}>
                ...
                <DialogContent>
                    <DialogContentText>
                        ¿Estás seguro de que quieres eliminar el perfil {rowToDelete?.perfil}?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
            ...
            <TableContainer component={Paper} sx={{ backgroundColor: '#2D2D44', margin:0, pagging:0}}>
                <Table>
                    <TableHead>
                        <TableRow>
                            <TableCell sx={{ color: '#fff', textAlign: 'center' }}>id</TableCell>
                            <TableCell sx={{ color: '#fff', textAlign: 'center' }}>Perfil</TableCell>
                        ...
                        {rows.map((row, index) => (
                            <TableRow
                               ...
                               <TableCell sx={{ textAlign: 'center' }}>{row.id}</TableCell>
                                <TableCell sx={{ textAlign: 'center' }}>{row.perfil}</TableCell>
